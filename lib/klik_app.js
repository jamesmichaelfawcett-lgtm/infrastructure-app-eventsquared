...
var KlikApp = function(uid, name, type, settings=APP_DEFAULT_SETTINGS, debug=APP_DEFAULT_DEBUG) {
    ...
    this.ios = (/iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream)
        // iPadOS 13+ Safari reports itself as desktop Mac Safari by default,
        // so the UA check above always misses real iPads. This detects that
        // case: 'MacIntel' + actual touch support = an iPad in disguise.
    ...
    window.addEventListener('online' , function(event) {
        self.online = true;
        self.update_connection_state();
        klik_hide_offline_banner();
        // silently try to pick the session back up now that we're online —
        // no re-prompt, no lost work, just quietly resumes
        if (self.api && self.api.get_token) {
            self.api.get_token();
        }
...
// ----------------------------------------------------------------
// offline banner — shown while the network is down, hidden once back.
// Credentials/session are never cleared just because wifi dropped;
// see klik_api.js's login() network-error branch.
// ----------------------------------------------------------------

function klik_offline_banner_node() {
    var node = document.getElementById('klik_offline_banner');
    if (!node) {
        node = document.createElement('div');
        node.id = 'klik_offline_banner';
        node.textContent = 'Offline — showing last known data';
        document.body.appendChild(node);
    }
    return node;
}

function klik_show_offline_banner() {
    klik_offline_banner_node().classList.add('visible');
}

function klik_hide_offline_banner() {
    klik_offline_banner_node().classList.remove('visible');
}


// ----------------------------------------------------------------
// read-only lock — used by the Map page. window.KLIK_READ_ONLY=true
// blocks every KlikApi write method (set/add/delete/pair) at the source
// in klik_api.js, so nothing can ever actually be saved to the server
// from this view, regardless of what UI path tries to trigger it.
// ----------------------------------------------------------------

var klik_readonly_toast_timer = null;

function klik_block_readonly_write() {
    var node = document.getElementById('klik_readonly_toast');
    if (!node) {
        node = document.createElement('div');
        node.id = 'klik_readonly_toast';
        node.textContent = 'View only — changes here aren\u2019t saved';
        document.body.appendChild(node);
    }
    node.classList.add('visible');
    if (klik_readonly_toast_timer) { clearTimeout(klik_readonly_toast_timer); }
    klik_readonly_toast_timer = setTimeout(function() {
