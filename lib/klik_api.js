        ...
        klik_http_post(self.environments[self.env].api + '/login', login_credentials,
            ...
            function(result) {
                ...
                if (result.status == 202) {
                    ...
                    // network error (wifi dropped / server unreachable) —
                    // this is NOT a bad password, so don't wipe the stored
                    // tokens or force a re-login prompt. Keep what we have,
                    // stay logged out of this particular request, and let
                    // the 'online' event trigger a silent retry.
                    self.cout('error', ':', 'no network connection — keeping cached credentials, will retry when back online');
                    self.set_logged_in(false);
                    if (typeof app !== 'undefined' && app.callbacks && app.callbacks.on_offline_auth_retry) {
                        app.callbacks.on_offline_auth_retry();
                    }
                }
...
KlikApi.prototype.set = function(namespace, data, callback) {
    if (window.KLIK_READ_ONLY) { klik_block_readonly_write(); return; }
